...
func main() {
	var a, v0, s0 float64
	var input string
	fmt.Println("Enter acceleration, initial velocity, and initial displacement, separated by commas:")
	fmt.Scan(&input)
	infoSlice := strings.Split(input, ",")
	fmt.Sscan(infoSlice[0], &a)
	fmt.Sscan(infoSlice[1], &v0)
	fmt.Sscan(infoSlice[2], &s0)
	fmt.Println(a, v0, s0)

	fn := GenDisplaceFn(a, v0, s0)
	fmt.Println("Enter time to calculate displacement. Enter 'q' to quit.")
	for {
		fmt.Scan(&input)
		if input == "q" || input == "Q" {
			break
